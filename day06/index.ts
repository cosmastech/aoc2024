...
const convertToGrid = (str: string): GameState => {
    ...
    return { personLocation: personLocation, personDirection: Direction.UP, grid: grid, spacesMoved: new Set() };
};
...
function findPath(gamestate: GameState) {
    ...
    gamestate.spacesMoved.add(`${gamestate.personLocation.x}__${gamestate.personLocation.y}`)
    let [nextX, nextY] = [gamestate.personLocation.x + dx, gamestate.personLocation.y + dy];
